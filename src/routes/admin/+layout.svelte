...
<div class="flex min-h-screen bg-bg">
	{#if !isLoginPage}
		...
		<!-- Sidebar -->
		<aside
			class="fixed inset-y-0 left-0 z-40 w-64 flex flex-col transform border-r border-zinc-800 bg-bg transition-transform lg:relative lg:translate-x-0
			{sidebarOpen ? 'translate-x-0' : '-translate-x-full'}"
		>
			<!-- Logo -->
			<div class="flex h-16 shrink-0 items-center justify-between border-b border-zinc-800 px-4">
				<div class="flex items-center gap-2">
					<img src="/logo.svg" alt="CloakBin" class="h-7 w-7" />
